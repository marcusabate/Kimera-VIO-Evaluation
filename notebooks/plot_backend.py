...
if not use_external_odom_as_gt:
    traj_ref_complete = pandas_bridge.df_to_trajectory(gt_df)
else:
    traj_ref_unaligned_rel = pandas_bridge.df_to_trajectory(gt_df)
    traj_ref_complete = convert_rel_traj_to_abs_traj(traj_ref_unaligned_rel)
...
if not use_external_odom_as_gt:
    ...
    gt_vel = np.linalg.norm(np.column_stack((gt_df['vx (kf)'], gt_df['vy (kf)'], gt_df['vz (kf)'])), axis=1)
...
if not use_external_odom_as_gt:
    ...
    traj_ref_unaligned_rel = pandas_bridge.df_to_trajectory(output_external_odom_df)
    traj_ref = convert_rel_traj_to_abs_traj(traj_ref_unaligned_rel)
...
#
# NOTE: Will give terrible results if you have set `use_external_odometry_as_gt` to `True` at the top.
